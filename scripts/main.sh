...
if [[ -f ./"${ARCH_PACKAGES_SCRIPT}"  &&  -x ./"${ARCH_PACKAGES_SCRIPT}"  && -f ./"${CUSTOM_CONF_SCRIPT}" && -x ./"${CUSTOM_CONF_SCRIPT}" && -f ./"${NVIDIA_MADNESS}" && -x ./"${NVIDIA_MADNESS}" ]]; then 
    . ./"${ARCH_PACKAGES_SCRIPT}" && . ./"${CUSTOM_CONF_SCRIPT}" && . ./"${NVIDIA_MADNESS}"
fi
sudo shutdown -h now
